...
@Component({
    selector: "atnd-equipment-property",
    templateUrl: "property.template.html",
    styleUrls: ["./property.template.scss"]
})
export class PropertyComponent {
  ...
  */
    ...
    export() {
        ...
        let thisComponent = this;
        this.projectService.getAllTable(param, (res) => {
            ...
            thisComponent.downloadTitle =  this.tabletitle+'_考勤统计报表_' + param['startDate'].split('-').join('')+'_'+param['endDate'].split('-').join('');;
            if(this.isIE){
    ...
    downloadTable(table, startDate, endDate) {
        ...
        return `<table>
                  <thead>
                      <tr>
                          <th colspan="15" >`+this.tabletitle+`_考勤统计报表</th>
                      </tr>
                      <tr>
